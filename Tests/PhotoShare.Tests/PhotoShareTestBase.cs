...
using PhotoShare.Migrations.SeedData;
...
namespace PhotoShare.Tests
{
    public abstract class PhotoShareTestBase : AbpIntegratedTestBase<PhotoShareTestModule>
    {
        ...
        protected void UsingDbContext(Action<PhotoShareDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<PhotoShareDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<PhotoShareDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<PhotoShareDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<PhotoShareDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PhotoShareDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<PhotoShareDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PhotoShareDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<PhotoShareDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<PhotoShareDbContext, Task<T>> func)
        {
            T result;
